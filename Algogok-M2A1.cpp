...
bool Banding(string a, string b) {
...
void UrutkanInventaris() {
    int n = (int)inventaris.size();
    for (int i = 1; i < n; i++) {
        Barang key = inventaris[i];
        int j = i - 1;

        while (j >= 0 && Banding(key.ID_Barang, inventaris[j].ID_Barang)) {
...
void SimpanDataCSV() {
    ...
    file.close();
    cout << "[SYSTEM]: " << inventaris.size() << " data telah berhasil disimpan ke" << namaFile << "!" << endl;
}
...
void GenerateSeratusDataDummy() {
    ...
    cout << "[DEV]: 100 Lapangan Pekerjaan telah dibuat!" << endl;
    cout << "[SISTEM]: Cek Menu 2 untuk melihat, lalu tekan Menu 6 untuk menyimpan ke CSV!\n" << endl;
}
